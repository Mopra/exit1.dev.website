...
**Step 1. Open the check on the Live page.** Make sure the check interval is sub-minute (30s on Nano, 15s on Pro). At 5-minute polling you'll miss the spike entirely.
...
- **Drop the check interval.** 15s checks (Pro) give you 4× the resolution of a 1-min check during an incident.
- **Enable the Boston region for the affected check.** Per-check region toggling on Nano and Pro. Two parallel probes mean you can disambiguate network path from server.
